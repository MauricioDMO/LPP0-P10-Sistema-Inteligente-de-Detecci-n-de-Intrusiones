...
export function SuricataOverviewPage() {
  ...
  return (
    <div className="flex flex-col gap-6 lg:gap-8">
      <section className="grid gap-4 lg:grid-cols-3" aria-label="Resumen de estado Suricata">
      ...
      <SectionCard eyebrow="Guía rápida" title="Flujo recomendado" description="Trabaja una sección a la vez para evitar mezclar cambios de distinta naturaleza.">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {workflow.map((item) => (
            <Link className="group rounded-xl border border-soc-outline/70 bg-soc-lowest/60 p-4 transition hover:-translate-y-0.5 hover:border-soc-primary/45 hover:bg-soc-blue/10" href={item.href} key={item.label}>
      ...
      <SectionCard eyebrow="Referencia" title="Acciones de override" description="Referencia rápida para decidir qué comportamiento asignar a un SID.">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {ACTIONS.map((action) => (
            <div className="rounded-xl border border-soc-outline/70 bg-soc-lowest/60 p-4" key={action}>
