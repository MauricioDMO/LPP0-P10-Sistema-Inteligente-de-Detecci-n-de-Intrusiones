...
if [[ ! -f "$ENV_FILE" ]]; then
  sudo cp "$PROJECT_DIR/gateway/scripts/gateway.env.example" "$ENV_FILE"
...
sudo ln -sf "$PROJECT_DIR/gateway/scripts/apply-gateway.sh" \
  /usr/local/sbin/suricata-gateway-apply

sudo ln -sf "$PROJECT_DIR/gateway/scripts/cleanup-gateway.sh" \
  /usr/local/sbin/suricata-gateway-cleanup

sudo ln -sf "$PROJECT_DIR/gateway/scripts/start-gateway.sh" \
  /usr/local/sbin/suricata-gateway-start

sudo ln -sf "$PROJECT_DIR/gateway/scripts/unmount.sh" \
  /usr/local/sbin/suricata-gateway-unmount

chmod +x "$PROJECT_DIR"/gateway/scripts/*.sh
